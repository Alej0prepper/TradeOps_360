...
mkdir -p .ci/data/evidence .ci/backup/filestore
...
COMMON=(--db_host="$HOST" --db_user="$USER_NAME" --db_password="$PGPASSWORD"
...
# Consistent synthetic backup with no application process writing in parallel.
...
# Restore only into a new disposable database; never overwrite a live database.
